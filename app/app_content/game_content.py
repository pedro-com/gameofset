...
class GamePanel(ctk.CTkFrame):
    ...
    def select_card(self, card: Tuple[int]):
        ...
        if card in self.selected_cards:
            ...
            return True
        if self.game.is_valid_selection(
            len(self.selected_cards)
        ) and self.game_id not in (Modes.INTERSET_GAME, Modes.SPC_GAME):
            return False
    ...
    def play_selection(self):
        ...
        cards = tuple(
            self.set_painter.generate_cards(
                self.content_panel, drawn_cards, card_function=self.select_card
            )
        )
        ...
        if self.game.is_game_end():
            if self.game_id == Modes.END_GAME and self.game.hold_card_remaining():
                ...
                while not found_card:
                    picker_window = self.set_painter.generate_picker_window(
                        self, self.game.n_attributes
                    )
    ...
    def start_game(self):
        ...
        card_positions = (
            (row, col)
            for row in range(self.content_rows - 1)
            for col in range(self.content_columns)
        )
        # Initialize game variables
        self.selected_cards = frozenset()
        table_cards = sample(tuple(self.game.table_cards), self.game.table_size)
        cards = self.set_painter.generate_cards(
            self.content_panel, table_cards, card_function=self.select_card
        )
        ...
        if not self.timer.is_end.trace_info():
            self.timer.is_end.trace_add(
                "write",
                lambda *args: self.game.modify_game_state(
                    timer_end=self.timer.is_end.get()
                ),
            )
        self.set_button.configure(state="disabled")
    ...
    def generate_content_panel(self):
        ...
        # Set content grid
        self.content_rows = (
            self.game.n_attributes + 1
            if self.game_id != Modes.SPC_GAME
            else self.game.n_attribute_values + 1
        )
        self.content_columns = self.game.n_attribute_values
        self.content_panel.rowconfigure(
            tuple(range(self.content_rows)), uniform="a", weight=1
        )
        self.content_panel.columnconfigure(
            tuple(range(self.content_columns)), uniform="a", weight=1
        )
        # Main game button
        font = ctk.CTkFont(
            family=GAME_BUTTON_FONT, size=GAME_BUTTON_TEXT_SIZE, weight="bold"
        )
        text = INTERSET if self.game_id == Modes.INTERSET_GAME else SET
        self.set_button = cw.custom_button(
            self.content_panel, text=text, font=font, command=self.play_selection
        )
        # For the SetPlanetComet gamemode
        if self.game_id == Modes.SPC_GAME:
            self.game.set_spc_button(self.set_button)
        self.set_button.grid(
            row=self.content_rows - 1,
            column=0,
            columnspan=self.content_columns,
            sticky="nsew",
            padx=PADX,
            pady=PADY,
        )
        self.content_panel.place(
            relx=0, rely=0, relwidth=1 - self.info_panel_rel_width, relheight=1
        )
    ...
    def generate_info_panel(self):
        self.info_panel = ctk.CTkFrame(
            self, fg_color=SECONDARY_COLOR, corner_radius=0
        ...
        label_font = ctk.CTkFont(
            family=OUTPUT_FONT, size=OUTPUT_TEXT_SIZE, weight="bold"
        )
        button_font = ctk.CTkFont(
            family=INFO_BUTTON_FONT, size=INFO_BUTTON_TEXT_SIZE, weight="bold"
        )
        option_font = ctk.CTkFont(
            family=OPTION_FONT, size=OPTION_SELECTION_SIZE, weight="bold"
        )
        # Score and timer
        self.score_meter = cw.ProgressTracker(
            self.info_panel,
            self.game.max_score,
            text_format=SCORE,
        ...
        self.timer = cw.Timer(
            self.info_panel,
            max_time=self.options[Options.GAME_TM],
            text_format=TIMER,
            progress_color=TIMER_COLOR,
        )
        self.game_label = cw.TextOutput(
            self.info_panel, text=REM_CARDS % (0,), font=label_font
        )
        # Info panels
        game_info_panel = cw.info_panel(
            self.info_panel,
            rel_start=(1, 0),
            rel_end=(0, 0),
            rel_width=1,
        ...
        self.option_panel = cw.SelectionPanel(
            self.info_panel,
            rel_start=(1, 0),
            rel_end=(0, 0),
            rel_width=1,
            ...
            button_font=option_font,
            option_dictionary=Modes.get_selection_options(self.game_id),
            accept_function=self._update_options,
        )
        # Info buttons
        how_to_play = cw.custom_button(
            self.info_panel, text=HOWTO, font=button_font, command=game_info_panel.move
        )
        restart_game = cw.custom_button(
            self.info_panel, text=RESTART, font=button_font, command=self.start_game
        )
        game_config = cw.custom_button(
            self.info_panel,
            text=OPTIONS,
            font=button_font,
            command=self.option_panel.move,
        )
        # Place the widgets
        ...
        self.info_panel.place(
            relx=1 - self.info_panel_rel_width,
            rely=0,
            relwidth=self.info_panel_rel_width,
            relheight=1,
        )
