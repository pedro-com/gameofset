...
def info_panel(
    ...
    dismiss_button = custom_button(
        slide_panel, text=UNDERSTOOD, font=font, command=dismiss_panel
    )
...
class SelectionBox(ctk.CTkFrame):
    def __init__(
        ...
        if len(option_names) != len(option_values):
            raise ValueError(
                f"Missing either option names[{option_names}] or option values[{option_values}]"
            )
        ...
        max_options = (
            len(option_names)
            if max_options < 0
            else min(len(option_names), max_options)
        )
        ...
        self.option_select = ctk.CTkOptionMenu(
            self, font=font, dropdown_font=font, values=self.option_names, anchor="w"
        )
    ...
    def _get_option_name(self, value):
        ...
        if opt_id == -1:
            if value not in self.option_names:
                raise ValueError(
                    f"Option {value} not in option names[{self.option_names}] or option values[{self.option_values}]"
                )
            return value
...
class SelectionPanel(SlidingFrame):
    ...
    def __init__(
        ...
        for idx, (opt_id, opt_values) in enumerate(option_dictionary.items()):
            self.option_dict[opt_id] = SelectionBox(
                self,
            ...
            self.option_dict[opt_id].grid(
                row=idx, column=0, columnspan=2, sticky="nsew", padx=PADX, pady=PADY
            )
        accept_button = custom_button(
            self,
            text=ACCEPT,
            font=button_font,
            command=self.accept_selection,
            hover_color=ACCEPT_COLOR,
        )
        cancel_button = custom_button(
            self,
            text=CANCEL,
            font=button_font,
            command=self.cancel_selection,
            hover_color=CANCEL_COLOR,
        )
...
class ExportPanel(SlidingFrame):
    def __init__(
        ...
        # Grid placement
        self.file_input.grid(
            row=0, column=0, columnspan=2, sticky="nsew", padx=PADX, pady=PADY
        )
        self.dir_input.grid(
            row=1, column=0, columnspan=2, sticky="new", padx=PADX, pady=PADY
        )
