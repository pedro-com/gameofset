...
class StructureDrawPanel(ctk.CTkFrame):
    ...
    def create_structure(self):
        cards = [
            card_picker.get_card()
            for card_picker in self.card_picker_buttons
            if card_picker.get_card() is not None
        ]
        # if len(cards) <= 1:
        #     return
        if self.painted_image:
            self.painted_image.destroy()
        self.painted_image = self.set_painter.generate_card_structure(
            self.content_panel, self.game.card_structure(cards), bg_color=BG_COLOR
        )
    ...
    def start_draw(self):
        ...
        for k in range(self.game.n_attributes + 1):
            picker_button = self.set_painter.generate_picker_button(
                self.info_panel,
                self.game.n_attributes,
                update_size_type="fit",
                button_text=SETPICK,
                command=self.check_card_selection,
            )
            picker_button.grid(
                row=k, column=1, columnspan=2, sticky="nsew", pady=CARD_PICKER_PADY
            )
    ...
    def generate_content_panel(self):
        ...
        self.generate_button = cw.custom_button(
            self.content_panel,
            text=GENERATE,
            font=font,
            state="disabled",
    ...
    def generate_info_panel(self):
        ...
        self.export_panel = cw.ExportPanel(
            self.info_panel,
            rel_start=(1, 0),
            rel_end=(0, 0),
            rel_width=1,
        ...
        # Buttons
        reset_button = cw.custom_button(
            self.info_panel, text=RESET, font=button_font, command=self.reset_buttons
        )
        self.download_button = cw.custom_button(
            self.info_panel,
            text=DOWNLOAD,
            font=button_font,
            state="disabled",
            command=self.export_panel.move,
        )
        how_to_use = cw.custom_button(
            self.info_panel,
            text=HOWTOUSE,
            font=button_font,
            command=game_info_panel.move,
        )
