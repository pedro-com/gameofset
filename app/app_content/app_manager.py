...
class AppModePanel(cw.SlidingFrame):
    def __init__(
        ...
        font = ctk.CTkFont(
            family=MENU_BUTTON_FONT, size=MENU_BUTTON_SIZE, weight="bold"
        )
        for app_mode, app_values in Modes.APP_MODES.items():
            button_option = cw.custom_button(
                self,
...
class App(ctk.CTk):
    ...
    def generate_header(self):
        header = ctk.CTkFrame(
            self, fg_color=PRIMARY_COLOR, corner_radius=0, border_color=ACCENT_COLOR
        )
        ...
        menu_button = cw.custom_button(
            header, text=MENU, font=title_font, command=self.mode_panel.move
        )
    ...
    def place_content(self, app_mode: str):
        ...
        if app_mode != Modes.STRUCTURE_DRAW:
            ...
            self.content_panel = StructureDrawPanel(
                self,
                instructions=app_mode_values["instructions"],
                options=app_mode_values["options"],
                info_panel_rel_width=self.info_panel_width,
        ...
        self.content_panel.place(
            relx=0,
            rely=self.header_height,
            relwidth=1,
            relheight=1 - self.header_height,
        )
