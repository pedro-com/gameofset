...
class SETPlanetComet(SETDeck):
    def __init__(self, n_attributes: int, n_attribute_values: int, set_score: int = 300, planet_score:int = 400, comet_score: int = 600):
    ...
    @property
    def max_score(self):
        max_sets = int(len(self.deck_cards) // (self.n_attribute_values)) * self.set_score
        max_planets = int(len(self.deck_cards) // ((self.n_attribute_values - 1)*2)) * self.planet_score
    ...
    def is_valid_selection(self, num_cards: int):
        ...
        if self.spc_button:
            self.spc_button.configure(text = COMET if is_comet else PLANET if is_planet else SET)
