...
class SETStructureDraw:
    ...
    def __init__(self,
        ...
        self.canvas_height_col = self.get_canvas_height_col(self.card_draw.n_attribute_values)
    ...
    async def adraw_cards(self, cards):
        async with TaskGroup() as tg:
            card_tgs = [tg.create_task(self.card_draw.adraw_card(card, self.card_bg)) for card in cards]
        image_canvas = Image.new('RGBA', (self.canvas_width, self.canvas_height_line), color = self.canvas_bg)
    ...
    async def adraw_card_list(self, cards:Tuple[int], card_bg:Tuple[int] = None):
        async with TaskGroup() as tg:
            task_images = [tg.create_task(self.card_draw.adraw_card(card, card_bg)) 
                for card in cards
            ]
    ...
    def generate_cards(self, master, cards: Iterable[Tuple[int]], bg_color:Any="white", is_button:bool=True, card_function:Callable[[Any], None]=lambda card: (), update_size_type='ratio'):
        card_images = self.draw_card_list(cards)
        return ((card, ImageCard(master, image=card_image, bg_color=bg_color, is_button=is_button, valid_selection=lambda card=card: card_function(card), update_size_type=update_size_type))
            for card, card_image in zip(cards, card_images)
        )
    
    def generate_card_structure(self, master, elements, bg_color:Any="white", update_size_type='ratio'):
        return ImageCard(master, image=self.draw_elements(elements), bg_color=bg_color, update_size_type=update_size_type)
    
    def _generate_card(self, master, card, bg_color=TRANSPARENT, update_size_type='ratio'):
        return ImageCard(master) if card is None else next(self.generate_cards(master, (card,), bg_color=bg_color, update_size_type=update_size_type))[1]
    
    def generate_card_picker(self, master, n_attributes:int, bg_color="white"):
        return CardPicker(master,
            n_attributes=n_attributes,
            n_attribute_values=self.card_draw.n_attribute_values,
            card_generator=lambda master, card : self._generate_card(master, card, bg_color=bg_color)
        )

    def generate_picker_window(self, master, n_attributes:int, bg_color="white", update_size_type='ratio', button_text=GUESS_CARD):
        return PickerWindow(master,
            n_attributes=n_attributes,
            n_attribute_values=self.card_draw.n_attribute_values,
            card_generator=lambda master, card : self._generate_card(master, card, bg_color=bg_color, update_size_type=update_size_type),
            button_text=button_text
        )
    
    def generate_picker_button(self, master, n_attributes:int, bg_color: str = "white", bd_color: str = None, bd_width: int = 0, update_size_type='ratio', button_text=GUESS_CARD, command: Callable[[None], None] = lambda : ()):
        return PickerButton(master,
            picker_window_generator=lambda : self.generate_picker_window(master, n_attributes, update_size_type=update_size_type, button_text=button_text),
            bg_color = bg_color,
            bd_color = bd_color,
            bd_width = bd_width,
            command=command
