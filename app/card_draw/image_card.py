...
class ImageCard(Canvas):
    ...
    def update_wait(self, event):
        ...
        self.prev_wait = self.after(0, lambda: self.update_size_ratio(event) if self.update_size_type == 'ratio' else self.update_size_fit(event))
...
class CardPicker(ctk.CTkFrame):
    def __init__(self, master, n_attributes:int, n_attribute_values:int,
        ...
        # Font card
        button_font = ctk.CTkFont(family = CARD_PICK_BUTTON_FONT, size = CARD_PICK_BUTTON_SIZE, weight = 'bold')
        option_font = ctk.CTkFont(family = CARD_INPUT_FONT, size = CARD_INPUT_SIZE, weight = 'bold')
        # Placements
        ...
        self.picker_slide = SelectionPanel(self,
            rel_start=(1, 0), rel_end=(0, 0), rel_height=1, rel_width=1,
        ...
        pick_card = custom_button(self, text=PICK, command=self.picker_slide.move, font=button_font)
        reset_card = custom_button(self, text=RESETPICK, command=self.reset_pick, font=button_font)
        # Place cards
    ...
    def selected_card(self):
        ...
        card = tuple(curr_picks[att_id]
            for att_id in self.pos_attributes 
        )
    ...
    def place_card(self):
        ...
        self.card_picked.grid(row = 0, column = 0, columnspan = 2, sticky = 'nsew', padx = PADX, pady = PADY)
...
class PickerWindow(ctk.CTkToplevel):
    def __init__(self, parent,
        ...
        self.card_input = CardPicker(self, n_attributes, n_attribute_values, fg_color, border_color, border_width, corner_radius, card_generator)
        button_font = ctk.CTkFont(family = CARD_PICK_BUTTON_FONT, size = CARD_PICK_BUTTON_SIZE, weight = 'bold')
        accept_input = custom_button(self, text = button_text, command = self.select, font = button_font)
